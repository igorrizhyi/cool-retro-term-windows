...
unix:DEFINES += HAVE_POSIX_OPENPT HAVE_SYS_TIME_H
macx:DEFINES += HAVE_UTMPX _UTMPX_COMPAT HAVE_PTSNAME HAVE_UNLOCKPT HAVE_GRANTPT

win32:LIBS += -lkernel32 -luser32 -ladvapi32 -lpsapi
...
win32 {
    QMAKE_POST_LINK = $(COPY_DIR) $$shell_path($$PWD/lib/color-schemes) $$shell_path($$DESTDIR/color-schemes) && \
        $(COPY_DIR) $$shell_path($$PWD/lib/kb-layouts) $$shell_path($$DESTDIR/kb-layouts) && \
        $(COPY) $$shell_path($$PWD/src/qmldir) $$shell_path($$DESTDIR) && \
        $(COPY) $$shell_path($$PWD/src/QMLTermScrollbar.qml) $$shell_path($$DESTDIR)
}
