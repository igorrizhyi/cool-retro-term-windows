...
#ifdef Q_OS_WIN
    if (exec.isEmpty()) {
        exec = QStandardPaths::findExecutable(QStringLiteral("pwsh"));
        if (exec.isEmpty())
            exec = QStandardPaths::findExecutable(QStringLiteral("powershell"));
        if (exec.isEmpty())
...
void Session::close()
{
    ...
    if (isRunning())
    {
        ...
        if (!_shellProcess->waitForFinished(1000))
        {
            ...
            if (!_shellProcess->waitForFinished(1000))
            {
                qWarning() << "Process " << processId() << " did not die";
