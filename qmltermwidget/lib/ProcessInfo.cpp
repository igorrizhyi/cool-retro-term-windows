...
#if defined(Q_OS_WIN)
WindowsProcessInfo::WindowsProcessInfo(int pid, bool readEnvironment)
    : ProcessInfo(pid, readEnvironment)
{
}

bool WindowsProcessInfo::readProcessInfo(int pid, bool /*readEnvironment*/)
{
    if (pid <= 0)
        return false;
    ...
    // Get the process name using OpenProcess + QueryFullProcessImageName
    HANDLE hProcess = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, static_cast<DWORD>(pid));
    if (hProcess) {
        WCHAR processName[MAX_PATH];
        DWORD size = MAX_PATH;
        if (QueryFullProcessImageNameW(hProcess, 0, processName, &size)) {
            QString fullPath = QString::fromWCharArray(processName, size);
        ...
        if (Process32FirstW(hSnapshot, &pe)) {
            do {
                if (static_cast<int>(pe.th32ProcessID) == pid) {
                    setParentPid(static_cast<int>(pe.th32ParentProcessID));
                    break;
                }
...
void WindowsProcessInfo::readUserName()
{
    WCHAR userName[256];
    DWORD userNameSize = 256;
    if (GetUserNameW(userName, &userNameSize)) {
        setUserName(QString::fromWCharArray(userName));
    }
}
#endif
...
QString SSHProcessInfo::format(const QString& input) const
{
    ...
    isIpAddress = hostAddress.setAddress(_host);
#endif
