...
#ifdef Q_OS_WIN
    // On Windows, ignore $SHELL (Unix concept, often set by Git Bash)
    // and use PowerShell or cmd.exe instead
    QString shellProg;
    {
        // Prefer pwsh (PowerShell 7+), then powershell (Windows PowerShell), then cmd
        ...
        if (!pwsh.isEmpty()) {
            ...
            if (!ps.isEmpty()) {
                ...
                shellProg = QStringLiteral("cmd.exe");
            }
        }
    }
#else
